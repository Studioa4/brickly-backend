...
app.get('/api/condomini', authMiddleware, async (req, res) => {
    const id_studio = req.user?.id_studio;
    try {
        const result = await pool.query('SELECT * FROM condomini WHERE id_studio = $1 ORDER BY denominazione', [id_studio]);
